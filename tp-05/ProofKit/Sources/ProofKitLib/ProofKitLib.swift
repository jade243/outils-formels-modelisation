...
public enum Formula {
    ...
    public var cnf: Formula {
      switch self.nnf {
      case .proposition(_) :                                      //Pour la proposition, c'est évident
        return self
      case .negation(_) :                                         //Pour la négation, c'est évident
        return self
      case .conjunction(let a, let b) :                           //Pour la conjonction, on appelle la récursion sur les 2 termes
        return a.cnf && b.cnf
      case .disjunction(let a, let b) :                           //Pour la disjonction :
        switch(a, b) {
        case (.conjunction(let c, let d), _) :
          return ((c.cnf || b.cnf).cnf && (d.cnf || b.cnf).cnf)                       //On distribue à droite
        case (_, .conjunction(let c, let d)) :
          return ((c.cnf || a.cnf).cnf && (d.cnf || a.cnf).cnf)                       //On distribue à gauche
        default :
          return a.cnf || b.cnf                                       //Sinon, il n'y a pas de distribution à faire
        }
      default :
        return self
      }
    ...
    public var dnf: Formula {
      ...
      case .negation(_) :                                         //Pour la négation, c'est évident
        return self
      case .disjunction(let a, let b) :                           //Pour la conjonction, on appelle la récursion sur les 2 termes
        return a.dnf || b.dnf
      case .conjunction(let a, let b) :                           //Pour la disjonction :
        switch(a, b) {
        case (.disjunction(let c, let d), _) :
          return ((c.dnf && b.dnf).dnf || (d.dnf && b.dnf).dnf)         //On distribue à droite
        case (_, .disjunction(let c, let d)) :
          return ((c.dnf && a.dnf).dnf || (d.dnf && a.dnf).dnf)         //On distribue à gauche
        default :
          return a.dnf && b.dnf                                         //Sinon, il n'y a pas de distribution à faire
        }
      default :
        return self
      }
